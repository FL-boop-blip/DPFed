...
def train_FedAvg(data_obj, act_prob, learning_rate, batch_size, epoch,
    ...
    clnt_x = data_obj.client_x;
    clnt_y = data_obj.client_y
    clnt_l = data_obj.client_l
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            for clnt in selected_clnts:
                ...
                for params in clnt_models[clnt].parameters():
                    params.requires_grad = True
                clnt_models[clnt] = train_model_avg(clnt_models[clnt], trn_x, trn_y,trn_l,
                                                    tst_x, tst_y,tst_l,
            ...
            ###
            loss_tst, acc_tst = get_acc_loss_AG_News(data_obj.test_x, data_obj.test_y,data_obj.test_l,
...
def train_FedProx(data_obj, act_prob, learning_rate, batch_size, epoch,
                  ...
                  save_period, weight_uniform=False, suffix='', trial=True, data_path='', rand_seed=0,
    ...
    if (trial) or (
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            traindata_cls_counts = np.asarray(
                [np.mean(data_obj.clnt_y[clnt] == cls) for clnt in range(data_obj.n_clnt) for cls in
                 range(data_obj.n_cls)]).reshape(-1, 10)
            for clnt in range(data_obj.n_clnt):
            ...
            distribution_difference = get_distribution_difference(traindata_cls_counts, selected_clnts, 'kl',
                                                                  global_dist)
...
def train_FedSMOO(data_obj, act_prob,
    ...
    if (trial) or (
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            df.to_csv("ndvar_values_FedSMOO_P2.csv", index=False)
...
def train_DPFedAvg(data_obj, act_prob, learning_rate, batch_size, epoch,
                   ...
                   model_func, init_model, sch_step, sch_gamma,
                   save_period, suffix='', trial=True, data_path='', rand_seed=0, lr_decay_per_round=1):
    suffix = 'DPFedAvg_' + str(act_prob) + suffix
    suffix += '_S%d_F%f_Lr%f_%d_%f_B%d_E%d_W%f' % (
        save_period, act_prob, learning_rate, sch_step, sch_gamma, batch_size, epoch, weight_decay)
        ...
        if saved_itr == -1:
            ...
            for clnt in clnt_models:  # 0 - n_client
                ...
                model.load_state_dict(copy.deepcopy(
                    dict(cur_cld_model.named_parameters())))  # use cloud model paramter to init local moedel
                for params in clnt_models[clnt].parameters():
                    params.requires_grad = True
            ...
            for clnt in selected_clnts:
                ...
                if i > (com_amount / 2) and T:
                    ...
                    clnt_models[clnt] = train_model_FedALS(model, model_func, cur_cld_model, alpha,
                                                           global_mdl, parameter_drifts_curr,
                                                           trn_x, trn_y, learning_rate * (lr_decay_per_round ** i),
                                                           batch_size, epoch, kd_T, print_per, weight_decay,
                                                           data_obj.dataset,
                                                           sch_step, sch_gamma)
                else:
...
def train_FedVARP(data_obj, act_prob, learning_rate, batch_size, n_minibatch,
    ...
    if (trial) or (not os.path.exists('%sModel/%s/%s/%dcom_sel.pt' % (data_path, data_obj.name, suffix, com_amount))):
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            delta_global_params = state_params[-1] + np.mean(
                (delta_params[selected_clnts] - state_params[selected_clnts]), axis=0)
...
def train_A_FedPD(data_obj, act_prob,
    ...
    if (trial) or (
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            avg_mdl_param_sel = np.mean(clnt_params_list[selected_clnts],
                                        axis=0)  # \overline{\theta^{t+1}} = \frac{1}{P}\sum_{i \in P_t} \theta_i^{t+1}
            for clnt in unselected_clnts:
    ...
    if (trial) or (
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            for clnt in selected_clnts:
                ...
                clnt_guide_models[clnt].global_model.load_state_dict(
                    copy.deepcopy(dict(cur_cld_model.named_parameters())))
                ...
                bb = eAM(i, 1e-4)
                print(f'aa is {aa}')
                clnt_models[clnt], clnt_guide_models[clnt], feat_s_pre, feat_t_pre = train_model_FedAKL(model,
                                                                                                        model_func,
                                                                                                        cur_cld_model,
                                                                                                        alpha,
                                                                                                        global_mdl,
                                                                                                        parameter_drifts_curr,
                                                                                                        trn_x, trn_y,
                                                                                                        learning_rate * (
...
def train_FedGamma(data_obj, act_prob, learning_rate, rho, batch_size, n_minibatch,
    ...
    df = pd.DataFrame(ndvar_value_list)
    df.to_csv("ndvar_values_FedGamma_P2.csv", index=False)
...
def train_MoFedSAM(data_obj, act_prob,
    ...
    if (trial) or (
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            df = pd.DataFrame(ndvar_value_list)
            df.to_csv("ndvar_values_FedSAM_P2.csv", index=False)
...
def train_FedSGAM_KL_AGNews(data_obj, act_prob,
                     learning_rate, batch_size, epoch, com_amount, print_per,
                     weight_decay, model_func, init_model, alpha_coef, rho,
                     sch_step, sch_gamma, save_period,
                     suffix='', trial=True, data_path='', rand_seed=0, lr_decay_per_round=1):
    ...
    # writer object is for tensorboard visualization, comment out if not needed
    writer = SummaryWriter('%sRuns_FedSGAM_KL_AGNews/%s/%s' % (data_path, data_obj.name, suffix[:12]))
    ...
    if (trial) or (
        ...
        for i in range(saved_itr + 1, com_amount):
            ...
            for clnt in selected_clnts:
                ...
                alpha_coef_adpt = alpha_coef / weight_list[clnt]  # adaptive alpha coef
                hist_params_diffs_curr = torch.tensor(hist_params_diffs[clnt], dtype=torch.float32, device=device)
                clnt_models[clnt] = train_model_FedSGAM_KL_AGNEWS(cur_cld_model, model, model_func, alpha_coef_adpt,
                                                           cld_mdl_param_tensor,
                                                           hist_params_diffs_curr,
                                                           trn_x, trn_y,trn_l, learning_rate * (lr_decay_per_round ** i),
                                                           batch_size, epoch, print_per, weight_decay,
                                                           data_obj.dataset, sch_step, sch_gamma)
            ...
            # df = pd.DataFrame(ndvar_value_list)
            # df.to_csv("ndvar_values_FedSGAM_KL_P2.csv", index=False)
