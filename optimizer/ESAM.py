...
class ESAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, rho, adaptive=True, **kwargs):
    ...
    # def step(self, loss_div,y_t,alpha=1.):
    def step(self, alpha=1.):
        ...
        loss_f_i = loss_func(predictions, labels)
        # loss_k_i = loss_div(predictions, y_t)
        loss=loss_f_i
        self.zero_grad()
        loss.backward()
